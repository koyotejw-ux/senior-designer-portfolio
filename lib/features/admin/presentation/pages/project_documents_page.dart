...
class _ProjectDocumentsPageState extends State<ProjectDocumentsPage> {
                  ...
                  children: [
                    ...
                    if (doc.isSpecial &&
                      ...
                      Padding(
                        ...
                        child: Text(
                          '17년의 실무 경력을 바탕으로 복잡한 비즈니스 요구사항을 명확한 UX/UI 설계로 구현하는 시니어 프로덕트 디자이너입니다. 단 한 번의 일정 지연 없는 책임감과 집요한 성실함으로 제조, 게임, 핀테크 등 다양한 분야에서 가시적인 성과를 만들어왔으며, 새로운 도구를 실무에 즉시 적용하는 능동적인 현업 디자이너로서의 역할을 지향합니다.',
                          style: AppTypography.bodySmall.copyWith(
                            color: AppColors.lightGray300,
                            height: 1.8,
                          ),
  ...
  String _generatePremiumHtmlForPdf(ProjectDocument doc) {
    ...
    buffer.writeln(
      '        <div class="contact-item"><strong>Exp</strong>17 Years (2009~Present)</div>',
    );
    buffer.writeln('      </div>');
    buffer.writeln('      ');
    buffer.writeln('      <div class="sidebar-section">');
  ...
  static List<DocumentSection> _getProfessionalResumeSections() {
    return [
      DocumentSection(
        ...
        content:
            '''17년의 실무 내공과 최신 AI 기술에 대한 집요한 연구를 결합하여 디자인의 미래를 제시하는 프로덕트 디자이너 정재웅입니다. 
...
AI 시대에 최적화된 도구 활용 능력과 17년 동안 단 한 번의 일정 지연도 없이 증명해온 변치 않는 성실함을 바탕으로 팀의 비즈니스 가치를 직접 증명합니다.''',
      ),
      DocumentSection(
...
• Technical Agility: Figma, Protopie, Flutter 등 기술적 도구를 가리지 않는 유연함과 17년의 깊이 있는 실무 노하우의 결합
  ...
  static List<DocumentSection> _getSelfIntroSections() {
    return [
      DocumentSection(
        ...
        content: '''17년의 경험은 제게 자만이 아닌, 새로운 기술을 누구보다 먼저 파헤칠 수 있는 끈기를 주었습니다.
...
17년 동안 지켜온 성실함과 AI 시대가 요구하는 기술력을 결합하여, 어제보다 압도적으로 나은 퍼포먼스를 내는 결과물로 보답하겠습니다.''',
      ),
    ];
  }
