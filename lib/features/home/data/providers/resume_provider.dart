...
final resumeProvider = Provider<ResumeModel>((ref) {
  return ResumeModel(
    ...
    coreCompetencies: const [
      '17년 1개월 차 UX/UI 전문가: 게임, 스마트홈(IoT), 금융, 공공 서비스 등 폭넓은 도메인 경험 보유',
...
final coverLetterProvider = Provider<CoverLetterModel>((ref) {
  return const CoverLetterModel(
    ...
    hobbies:
        '일정 준수율 100%의 책임감과 기술적 유연성\n\n저의 가장 큰 자산은 \'프로젝트 일정 준수율 100%\'라는 기록이 증명하는 강력한 책임감입니다. 촉박한 론칭 일정이나 예기치 못한 환경적 변수 속에서도 단 한 번의 중도 이탈 없이 완성도 높은 결과물을 만들어왔습니다. 또한, 17년 1개월이라는 긴 시간 동안 Flash에서 Figma로, 다시 AI 기반의 프로세스로 급변하는 기술 트렌드를 스스로 학습하고 즐기는 유연함을 갖추고 있습니다.\n\n단점으로는 완벽한 완성도를 추구하느라 기획 검토 단계에서 많은 공수를 들이는 편이지만, Figma 컴포넌트 표준화 and AI 툴을 적극 도입해 제작 공수를 획기적으로 줄임으로써 속도와 품질을 동시에 완벽히 통제하고 있습니다.',
    aspiration:
        '성과로 존재 가치를 증명하는 핵심 프로덕트 디자이너\n\n귀사에 합류한다면 17년 1개월간 쌓아온 폭넓은 실무 경험과 통찰력을 바탕으로, 비즈니스 목표를 가장 빠르고 효율적으로 달성할 수 있는 UI/UX 솔루션을 제시하겠습니다. 연차에 기대어 관리 업무에만 치중하기보다, 현장에서 직접 발로 뛰며 Figma 스펙을 정교화하고 기획 및 개발 파트와 기민하게 소통하는 \'대체 불가능한 핵심 실무자\'의 역할을 수행하겠습니다.\n\n비즈니스와 개발 프로세스를 모두 이해하는 디자이너로서, 제 전문성이 귀사 서비스의 가시적인 지표 개선과 전환율 상승으로 직결될 수 있도록 매 순간 결과물로 그 가치를 증명하겠습니다.',
  );
});
